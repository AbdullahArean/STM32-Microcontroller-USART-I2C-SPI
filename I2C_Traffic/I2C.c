...
*/
	...
	//2.Configure the I2C PINs for ALternate Functions
	GPIOB->MODER |= (2<<16);  // Bits (17:16)= 1:0 --> Alternate Function for Pin PB8
	GPIOB->MODER |= (2<<18);  // Bits (19:18)= 1:0 --> Alternate Function for Pin PB9
	...
	//3.Reset the I2C (Software Reset) 
	I2C1->CR1 |= (1<<15);
	I2C1->CR1 &= ~(1<<15);
	...
	//5.Configure the clock control registers(123)
	I2C1->CCR = 225<<0; 
...
*/
	...
	I2C2->CR2 |= (45<<0);  // PCLK1 FREQUENCY in MHz
	
	// Configure the clock control registers
	I2C2->CCR = 225<<0;  // check calculation in PDF
	
	// Configure the rise time register
	I2C2->TRISE = 46;  // check PDF again
  ...
  I2C2->OAR1 &= ~(1<<15);
	I2C2->OAR1 |= (0x44<<1); // Set slave address
	
	I2C2->CR1 |= I2C_CR1_PE; //enable i2c2

  I2C2->CR2 |= I2C_CR2_ITEVTEN; // Enable event interrupt
...
*/	
	...
	{
		while (!(I2C1->SR1 & (1<<7)));  // wait for TXE bit to set 
		char ch=(uint32_t )*data++;
		if(ch=='\0' || ch=='\n') 
		{
			 break;
		}
		I2C1->DR = ch;  // send data
	}
